...
string FILE_path("/bin/object_test.bnf");
...
int iatalk()
{
	const char *login_config    = "appid = 5d07aecd"; //登录参数
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
...
{
	printf(path1.data());
	ros::init(argc, argv, "castle_xf_order_node");    //初始化节点，向节点管理器注册
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("/voice/castle_xf_asr_topic", 1, orderCallback);
	...
	ros::Publisher cmd_pub = n.advertise<std_msgs::Int32>("/voice/castle_xf_cmd_topic", 1);
