...
#define IVW_AUDIO_FILE_NAME "/home/kyle/voice_ws/src/xf_awake/bin/audio/awake.wav"
...
string temp_path;
string pkg_path = ros::package::getPath("castle_voice_system");
string fo_("fo|");
...
int main(int argc, char* argv[])
{
	...
	ros::init(argc, argv, "castle_xf_awake_node");    //初始化节点，向节点管理器注册
	ros::NodeHandle n;
	ros::Publisher pub = n.advertise<std_msgs::Int32>("/voice/castle_xf_asr_topic", 1);
	...
	*/
	...
	nh.param("lgi_param", my_lgi_param, std::string("appid = 5c6b6548,work_dir = ."));    //从launch文件获取参数
...
exit:
	MSPLogout(); //退出登录
	printf("---------------------------------------------------------------\n");
	printf("Turn off Castle's voice wakeup feature.....\n");
	printf("---------------------------------------------------------------\n");
	return 0;
}
