...
class XML_Analysis():
    def __init__(self):
        ...
        #在launch文件中获取参数
        self.file_path = rospy.get_param("~file_path", "/params/objectDetect.xml")
        self.sub = rospy.Subscriber('/voice/castle_xf_cmd_topic', Int32 , self.cmd_callback)
        self.pub = rospy.Publisher('/voice/object_color', Int64, queue_size = 1)
        self.DOMTree = None
        self.nlp = None
        self.version = None
        self.rawtext = None
    ...
    def analysis_file(self):
        ...
        if self.nlp.hasChildNodes():
            ...
            try:
                self.color = self.object.getElementsByTagName('color')[0]
                print "color: %s" % self.color.childNodes[0].data
                if self.color.hasAttribute("id"):
                    self.color_id = int(self.color.getAttribute("id"))
                    print "color id: %s" %  self.color.getAttribute("id")
            except IndexError:
                self.color = None
                self.color_id = None
                print 'No color'
    ...
    def Process_Speech_cmd_to_Speed(self):
        ...
        if self.confidence_data>=15:
            ...
            self.goal_point_msg.data = self.color_id
